...
Return an integer array answer where answer[0] is the number of candies in the box that Alice must exchange, and answer[1] is the number of candies in the box that Bob must
...
class Solution:
    def fair_candy_swap(self, alice_sizes: List[int], bob_sizes: List[int]) -> List[int]:
        alice_total, bob_total, distinct_bob_sizes = sum(alice_sizes), sum(bob_sizes), set(bob_sizes)
        exchange = int((bob_total - alice_total) / 2)

        for alice_size in alice_sizes:
            bob_size = alice_size + exchange

            if bob_size in distinct_bob_sizes:
