...
class SparseVector:
    ...
    def __init__(self, nums: list[int]):
        self.nonzeros = {idx: num for idx, num in enumerate(nums) if num > 0}

    def dot_product(self, vec: "SparseVector") -> int:
        common_keys = self.nonzeros.keys() & vec.nonzeros.keys()
        return sum(self.nonzeros[key] * vec.nonzeros[key] for key in common_keys)
