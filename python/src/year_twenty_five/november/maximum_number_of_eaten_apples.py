...
class Solution:
    def eaten_apples(self, apples: list[int], days: list[int]) -> int:
        rot_days, freq = [], defaultdict(int)
        n, answer = len(apples), 0

        for i in range(max(i + d for i, d in enumerate(days))):
            rot_day = i + days[i] if i < n else i
            apple = apples[i] if i < n else 0

            freq[rot_day] += apple
            heapq.heappush(rot_days, rot_day)

            while rot_days and (rot_days[0] <= i or freq[rot_days[0]] == 0):
                heapq.heappop(rot_days)
