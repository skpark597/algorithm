...
class Solution:
    def assign_bikes(self, workers: list[list[int]], bikes: list[list[int]]) -> int:
        answer, n, m = sys.maxsize, len(workers), len(bikes)
        dp = [sys.maxsize] * 2**m

        dp[0] = 0

        for k in range(0, n):
            for comb in combinations(range(m), k + 1):
                mask = sum(1 << i for i in comb)

                for i in comb:
                    dist = abs(workers[k][0] - bikes[i][0]) + abs(workers[k][1] - bikes[i][1])
                    dp[mask] = min(dp[mask], dist + dp[mask & ~(1 << i)])
