 ...
 *  (possibly zero) elements of b. Two subsets are considered different
...
function countMaxOrSubsets(nums: number[]): number {
  let answer = 0;
  const goal = nums.reduce((p, c) => p | c);

  for (let m = 0; m < 1 << nums.length; ++m) {
    let current = 0;

    for (let j = 0; j < nums.length; ++j) {
      if ((m >> j) & 1) current |= nums[j];
    }

    if (current == goal) answer++;
  }
