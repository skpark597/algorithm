...
function findLHS(nums: number[]): number {
  ...
  for (const [key, value] of Object.entries(countMap)) {
    ...
    if (countMap[+key - 1]) {
      caseKeyIsMax = value + countMap[+key - 1];
    }

    if (countMap[+key + 1]) {
      caseKeyIsMin = value + countMap[+key + 1];
    }

    lengthOfLHS = Math.max(lengthOfLHS, caseKeyIsMax);
    lengthOfLHS = Math.max(lengthOfLHS, caseKeyIsMin);
  }
